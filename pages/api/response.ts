...
export default async function ResponseApi(
  req: NextApiRequest,
  res: NextApiResponse
) {
  ...
  try {
    ...
    if (!querySnapshot.empty) {
      ...
      console.log("Ticket with timestamp:" + timestamp + " has been updated with response: " + resp);
      res.status(200).json({ message: "success" });
    } else {
      console.log("Ticket with timestamp:" + timestamp + " not found.");
